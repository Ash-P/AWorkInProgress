...
public class AddABook {
	...
	StringBuilder pagesOnDateToStore = new StringBuilder();
		//manual specified pagesReadOnADate entries are stored here when the addBtn is pressed
	...
	private void createUI() {
		...
		dateStartedPicker.setEditable(false); //forces a date to be selected from the calendar (prevents future dates being entered manually)
		...
		dateCompletedPicker.setEditable(false); //forces a date to be selected from the calendar (prevents future dates being entered manually)
		...
		setupGridElement(statusBox, bookTitleTxt, bookPagesTxt, dateStartedPicker, dateCompletedPicker, pagesReadTxt, previouslyReadBox, dateReadPicker, pagesOnDateTxt, addBtn,
				additionalBookInfoBtn, bookAuthorTxt, bookPublisherTxt, bookPublicationYearTxt, bookGenreTxt, bookDescriptionTxt);
	}
	...
	private void addNewPreviousBook(String title, int pages, String dateStarted, String dateCompleted, String pagesReadOnADate, String author, String publisher, int publicationYear, String genre, String description) {
		...
		alldata.userStore.incrementTotalPagesRead(pages);
		alldata.userStore.incrementTotalBooksRead(1);
	...
	private void setupHandlers() {
		...
		submitBtn.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				if(!bookTitleTxt.getText().isEmpty()) title = bookTitleTxt.getText();
				else isValid = false;
				...
				if(!bookAuthorTxt.getText().isEmpty()) author = bookAuthorTxt.getText();
				if(!bookPublisherTxt.getText().isEmpty()) publisher = bookPublisherTxt.getText();
				if(!bookGenreTxt.getText().isEmpty()) genre = bookGenreTxt.getText();
				if(!bookDescriptionTxt.getText().isEmpty()) description = bookDescriptionTxt.getText();
		...
		addBtn.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				if(!(dateReadPicker.getValue() == null)) dateRead = dateReadPicker.getValue().format(DATE_FORMATTER).toString();
				else isValid = false;
				...
				if(statusBox.getValue() == "Read Previously") {
					...
					if(isValid && BookDataValidation.validateProgressFieldsPast(pages, dateStarted, dateCompleted, dateRead, pagesOnDate, pagesReadOnADate)) {
						...
						createAlert(AlertType.INFORMATION, "Validation Successful", "New reading progress has been addded", "Added " + pagesOnDate + " pages to the system.");
						System.out.println(dateRead + "," + pagesOnDate + " "); //Test
						clearFields();
					}
					else createAlert(AlertType.ERROR, "Validation Unsuccessful", "Validation Unsuccessful", "Reading progress has not been successfully validated.");
				}
	 ...
	 * @param pages : the number of pages to distribute over the range
	 * @param startDate : first date of the range
	 * @param endDate : last date of the range
	 * 
	 * @return pagesReadOnADate : all 'date,value' pairs as a String
	 * 
	 * NOTE: any remainder of pages not evenly distributed MUST be added to the LAST ENTRY (i.e. with date endDate), as shown by pagesOnLastDay below.
	 * This is because @param pages and the sum of all 'values' in @return pagesReadOnADate MUST be equal
	 */
	public String distributeEvenly(int pages, LocalDate startDate, LocalDate endDate) {
		String pagesReadOnADate = "";
		pagesReadOnADate += endDate.format(DATE_FORMATTER) + "," + pages + " "; //Test
		/*
