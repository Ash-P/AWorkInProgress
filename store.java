...
public class store implements storage{
	...
	public ArrayList<userData> retrieveUserData() throws IOException {
    	...
    	//for(String s = userFileReader.readLine(); s != null; s = userFileReader.readLine()){
		while(s!=null) {
			...
			try{userdata.booksCompletedOnADate = properties[6];}catch(Exception e) {userdata.booksCompletedOnADate = "";};
	...
	public ArrayList<bookData> retrieveBookData() throws IOException{
		...
		BufferedReader input = new BufferedReader(fileIn);
    	//for(String s = bookStoreReader.readLine(); s != null; s = bookStoreReader.readLine()){
		String s = input.readLine();
		while (s!=null) {
			String[] properties = s.split(" ; ");
    	    bookData bookdata = new bookData();
    	    
    	    if(properties[0] == " ") {}else { bookdata.bookID = Integer.parseInt(properties[0]);}
    
    	    if(properties[0] == " ") {}else { bookdata.status = Integer.parseInt(properties[1]);}

			
			bookdata.title = properties[2];
			
			if(properties[0] == " ") {}else { bookdata.pages = Integer.parseInt(properties[3]);}
	...
	public ArrayList<targetData> retrieveTargetData() throws IOException{
		...
		String s = input.readLine();
    		...
    		try {t.targetID = Integer.parseInt(properties[0]);}catch(Exception e){t.targetID = -1;}
    	
    		try {t.targetType = Integer.parseInt(properties[1]);}catch(Exception e){t.targetType = -1;}

    		try {t.isComplete = Boolean.parseBoolean(properties[2]);}catch(Exception e){t.isComplete = false;}
		
    		try {t.deadlineDate = properties[3];}catch(Exception e){t.deadlineDate = "";}
			
    		try {t.bookID = Integer.parseInt(properties[4]);}catch(Exception e){t.bookID = -1;}
			
    		try {t.targetValue = Integer.parseInt(properties[5]);}catch(Exception e){t.targetValue = -1;}
	
    		try {t.valueRemaining = Integer.parseInt(properties[6]);}catch(Exception e){t.valueRemaining = -1;}
	...
	public void storeBookData(ArrayList<bookData> newBookData){
		try {
			...
			for(bookData b : newBookData){
				updateFile.append(b.bookID+" ; "+b.status+" ; "+b.title+" ; "+b.pages+" ; "+b.dateAdded+" ; "+b.dateStarted+" ; "+b.dateCompleted+" ; "+b.author+" ; "+b.publisher+" ; "+b.publicationYear+" ; "+b.genre+" ; "+b.description+" ; "+b.pagesRead+" ; "+b.pagesReadOnADate+"\n");
			}
			updateFile.close();
	...
	public void storeTargetData(ArrayList<targetData> newTargetData){
		try{
			...
			for(targetData t : newTargetData){
				updateFile.append(t.targetID+" ; "+t.targetType+" ; "+t.isComplete+" ; "+t.deadlineDate+" ; "+t.bookID+" ; "+t.targetValue+" ; "+t.valueRemaining+"\n");
			}
			updateFile.close();
